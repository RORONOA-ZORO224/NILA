...
class ToolRouter:
    ...
    async def execute(self, intent: IntentResult, raw_command: str) -> dict[str, Any]:
        ...
        Registers undo handlers where applicable.
        Returns a result dict with status, summary, and action_id.
        """
        ...
        if action == "send_email":
            ...
            result = {
                ...
                "message": f"I understood your command but don't know how to handle action '{action}' yet.",
            }

        # Log to history and kick off persona learning (non-blocking)
    ...
    async def _handle_send_email(self, intent: IntentResult, prefs: dict) -> dict:
        ...
        if result.get("status") == "sent":
            ...
            undo_shield.register(
                action_id=0,  # Will be updated by caller after history.log()
                summary=f"Email to {recipient}: '{subject}'",
    ...
    async def _handle_slack(self, intent: IntentResult, prefs: dict) -> dict:
        ...
        if result.get("status") == "sent":
            ...
            undo_shield.register(
                action_id=0,
                summary=f"Slack message in {channel}",
    ...
    async def _handle_chain(self, intent: IntentResult, prefs: dict, raw_command: str) -> dict:
        """Execute a multi-step workflow. Each step is processed independently."""
        results = []
        for step in intent.chain_steps:
            step_action = step.get("action", "other")
            # Build a minimal IntentResult for the sub-step
